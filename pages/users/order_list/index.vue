<template>
	<view>
		<view class="headertop">
			<u-subsection :list="list" mode="subsection" activeColor="#DCA842" :current="current"
				@change="sectionChange"></u-subsection>
		</view>
		<!-- 占位 -->
		<u-gap height="35" bgColor="#FFF"></u-gap>
		<view class='my-order'>
				...
				<view class='item' v-for="(item,index) in orderList" :key="index">
					...
					<view class='bottom acea-row row-right row-middle'
						...
						<view class='bnt bg-color' v-else-if="item.status== 0 || item.status== 1 || item.status== 3"
							@click='goOrderDetails(item.id)'>查看详情</view>
						<view class='bnt bg-color' v-else-if="item.status==2" @click='goOrderDetails(item.id)'>去评价
						</view>
	...
	export default {
		...
		methods: {
			...
			goOrderDetails(order_id) {
				...
				// })
				uni.navigateTo({
					url: `/pages/goods_details/index?id=${order_id}`
				})
			},
			/**
			 * 切换类型
