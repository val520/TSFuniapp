<template>
	<view>
					...
					<view v-for="(item,index) in collectProductList" :key="index" class='item acea-row row-middle'>
						...
						<navigator :url='"/pages/goods_details/index?id="+item.commodityId' hover-class='none'
							...
							<view>
								...
								<view class='money font-color'>￥{{item.sellPrice}}</view>
							</view>
						</navigator>
					</view>
				</checkbox-group>
		...
		<view class='noCommodity' v-else-if="collectProductList.length ===0">
			<view class='pictrue'>
			...
			getusercolectproduct() {
				...
				this.$myRequest({
					...
					if (res.data.code === 200) {
						...
						let collectProductList = res.data.result.records;
						this.collectProductList = this.$util.SplitArray(collectProductList, this
							.collectProductList);
						this.$set(this, 'collectProductList', this.collectProductList);
					} else {
