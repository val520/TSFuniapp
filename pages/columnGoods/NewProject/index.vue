		...
		<view class="box">
			<u--form labelPosition="left" :model="model1" labelWidth="80" :rules="rules" ref="form1">
				<u-form-item label="项目名称" required="true" prop="userInfo.projectName" borderBottom ref="item1">
					<u--input v-model="model1.userInfo.projectName" border="none" placeholder="请输入"></u--input>
				</u-form-item>
...
<script>
	...
	export default {
		...
		onShow() {
			...
			this.projectObjlist()
			//默认匹配当前用户的组织机构
			this.model1.userInfo.orgname = uni.getStorageSync("userInfo").orgCodeTxt
			this.model1.userInfo.orgId = uni.getStorageSync("userInfo").departIds
			// this.queryTreeList()
		},
		onReady() {
		...
		methods: {
			...
			change(e) {
				console.log(e);
				this.$refs.form1.validateField('userInfo.projTypeId')
			},
			// 点击跳转地图
			mapClick() {
				uni.$u.toast('打开地图')
