	...
	export default {
		...
		methods: {
			...
			orderList() {
				...
				this.$myRequest({
					url: `/tsf/tsfStoreCart/wxList`,
					method: "get",
					data: val
				}).then(res => {
					if (res.data.code === 200) {
						res.data.result.records.forEach((key) => {
							key.attrStatus = key.id
						})
						this.cartList.valid = res.data.result.records
						for (let i = 0; i < Math.ceil(res.data.result.records.length / 9999); i++) {
							// let cartList = await this.getCartData(data);
							// let valid = cartList.list;
							let validList = this.$util.SplitArray(valid, this.cartList.valid);
							let numSub = [{
								numSub: true
							}, {
								numSub: false
			...
			deleteshopne(e) {
				// let val = []
				// e.forEach((key) => {
				// 	e.push(key.commodityId)
				// })
				this.$myRequest({
					url: "/tsf/tsfStoreCart/deleteBatch",
					method: "POST",
					data: e
				}).then(res => {
					if (res.data.code === 200) {
					...
					for (let index in validList) {
						if (that.inArray(validList[index].id, selectValue)) {
							selectCountPrice = that.$util.$h.Add(selectCountPrice, that.$util.$h.Mul(validList[index]
								.cartNum, validList[index].sellPrice))
						}
					}
					that.selectCountPrice = selectCountPrice;
				}
