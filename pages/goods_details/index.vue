									...
									<swiper-item v-for="(item,indexw) in good_list" :key="indexw">
										<view class="list acea-row row-middle" :id="'list'+indexw">
											<view class="item" v-for="(val,indexn) in item.list" :key="indexn"
												...
												<view class="money font-color">¥{{val.sellPrice}}</view>
											</view>
										</view>
									</swiper-item>
								</swiper>
		...
		<view class='footer acea-row row-between-wrapper'>
			<block v-if="type === 'normal'">
				...
				<view class="bnt acea-row" v-else>
					<form @submit="joinCart(productInfo,0)" report-submit="true"><button class="joinCart bnts"
							form-type="submit">加入购物车</button></form>
					<form @submit="goBuy(productInfo,1)" report-submit="true"><button class="buy bnts"
			...
			<view class="box">
				...
				<view class="centernum">
					...
					<view style="text-align: center;">
						<input :value="buyNum" class="uni-input" type="number" @input='popnum' />
					</view>
					<view class="addnum" @click="upnum(markerValue)">
						+
					</view>
				</view>
		...
		methods: {
			...
			ChangeAttr: function(res) {
				...
				if (productSelect) {
					...
					this.$set(this.attr.productSelect, "price", productSelect.totalAmount);
					this.$set(this.attr.productSelect, "stock", productSelect.totalAmount);
					this.$set(this.attr.productSelect, "unique", productSelect.id);
					this.$set(this.attr.productSelect, "cart_num", 1);
					this.$set(this, "attrValue", res);
					this.$set(this, "attrTxt", "已选择");
				} else {
					this.$set(this.attr.productSelect, "image", this.productInfo.totalAmount);
					this.$set(this.attr.productSelect, "price", this.productInfo.totalAmount);
			...
			getGoods() {
				...
				this.$myRequest({
					...
					if (res.data.code === 200) {
						let good_list = res.data.result.records || [];
			...
			getGoodsDetails(e) {
				...
				this.$myRequest({
					...
					if (res.data.code === 200) {
						this.sliderImage = res.data.result.records[0].slideAttIds
						this.productInfo = res.data.result.records[0]
						this.mountNew = res.data.result.records[0].forSellAmount
						// 是否被收藏
						this.$set(this, 'userCollect', res.data.result.records[0].collectFlag);
						// this.$set(this.attr, 'productAttr', res.data.result.records[0]);
						this.attr.productAttr = res.data.result.records[0]
					} else {
						uni.$u.toast(res.data.message)
					}
				})
			...
			DefaultSelect: function() {
				...
				if (productSelect && productAttr.length) {
					...
					this.$set(this.attr.productSelect, "image", this.productInfo.coverAttIds[0].interRqUrl);
					this.$set(this.attr.productSelect, "price", productSelect.totalAmount);
					this.$set(this.attr.productSelect, "stock", productSelect.totalAmount);
					this.$set(this.attr.productSelect, "unique", productSelect.id);
					this.$set(this.attr.productSelect, "cart_num", 1);
					...
					this.$set(this.attr.productSelect, "image", this.productInfo.coverAttIds[0].interRqUrl);
					this.$set(this.attr.productSelect, "price", this.productInfo.totalAmount);
					this.$set(this.attr.productSelect, "stock", this.productInfo.totalAmount);
					this.$set(
						this.attr.productSelect,
						"unique",
						this.productInfo.id || ""
			...
			setCollect: function() {
				if (this.userCollect) {
					// 取消收藏
					this.$myRequest({
						url: "/tsf/tsfCommodityCollect/deleteBatch",
						method: "post",
					...
					// 添加收藏
					this.$myRequest({
						url: "/tsf/tsfCommodityCollect/add",
						method: "post",
						data: val
					}).then(res => {
						if (res.data.code === 200) {
			...
			upnum(e) {
				...
				if (this.buyNum >= e.forSellAmount) {
					uni.showToast({
						title: `不能大于库存`,
						icon: "none"
					})
			...
			// 校验输入的数字是否大于库存
			popnum(e) {
				this.buyNum = parseInt(e.detail.value)
				if (this.buyNum > this.mountNew) {
					uni.showToast({
						title: `不能大于库存`,
						icon: "none"
					})
