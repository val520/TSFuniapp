	...
	<view ref="node" name="LyTreeNode" v-show="node.visible" class="ly-tree-node" :class="{
	 ...
	 role="treeitem" @tap.stop="handleClick">
		<view class="ly-tree-node__content" :class="{
			...
			<block v-if="showActions">
				<view>
					<text>【</text>
					<text class="ly-tree-node__extbutton">修改</text>
					<text>】</text>
				</view>
				<view>
					<text>【</text>
					<text @tap.stop="handleExtButtonClick('newchildren')" class="ly-tree-node__extbutton">新增下级</text>
					<text>】</text>
				</view>
				<view>
					<text>【</text>
		...
		<view v-if="!renderAfterExpand || childNodeRendered" v-show="expanded" class="ly-tree-node__children" role="group">
			<ly-tree-node v-for="cNodeId in node.childNodesId" :nodeId="cNodeId" :render-after-expand="renderAfterExpand"
			 :show-checkbox="showCheckbox" :show-radio="showRadio" :showActions="showActions" :check-only-leaf="checkOnlyLeaf" :key="getNodeKey(cNodeId)"
