<template>
	<view :class="modal?'show-qrcode':'hide-qrcode'">
		<view class="box-qrcode" @longtap="longtapCode" @click="longtapCode">
	...
	export default {
		...
		methods: {
			...
			crtQrCode() {
				...
				// #endif
				//#ifdef MP
				uni.createSelectorQuery().in(that).select(".canvas-qrcode").boundingClientRect(data => {
					console.log(JSON.stringify(data))
					that.createQrCode(this.url, that.qrcode_id, data.width, data.height, that.themeColor, that
						.is_themeImg, that.themeImg,
						that.h_w_img);
				}).exec()
				// #endif
